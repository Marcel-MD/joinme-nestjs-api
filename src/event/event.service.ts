...
export class EventService {
    ...
    private checkCategory(category: string): boolean {
        ...
        if (!enumValue)
            throw new NotFoundException([`Event with category '${category}' not found`])
        return true;
    }
    ...
    public async findByCategory(category: string): Promise<Event[]> {
        ...
        return await this.eventRepository.find({ where: { category: Category[category] } })
    }

    public async findById(id: number): Promise<Event> {
        this.logger.debug(`Getting event by id ${id}`);
        const event = await this.eventRepository.findOne({ where: { id }, relations: ['attendees'] })
    ...
    public async update(input: UpdateEventDto, id: number, user: User): Promise<Event> {
